...
### Vinculación Google + contraseña

Cuando una cuenta ingresa con Google y todavía no tiene el proveedor `password`, Taskify solicita crear una contraseña adicional. Esa credencial se vincula a la misma cuenta Firebase, por lo que se conserva el mismo UID y las mismas tareas de Firestore. Después de configurarla, el usuario puede ingresar con Google o con email + contraseña.

La contraseña agregada es una credencial de Taskify/Firebase; no es la contraseña de Gmail.
...
Envío de resumen de tareas mediante Gmail SMTP + Nodemailer desde una Vercel Function.
...
`GMAIL_USER` y `GMAIL_APP_PASSWORD` son privadas del servidor y no deben usar el prefijo `VITE_`. `FIREBASE_API_KEY` puede reutilizar el mismo valor público de `VITE_FIREBASE_API_KEY`.
...
- Envío de resumen por Gmail SMTP + Nodemailer
...
│   ├── emailService.ts       # Gmail SMTP + Nodemailer
...
2. Confirma que el email está verificado en Gmail SMTP + Nodemailer (modo sandbox)
...
Envío de emails con Gmail SMTP + Nodemailer
